...
class RTMPose:
    def __init__(self, onnx_file, device="cpu"):
        self.sess = self.build_session(onnx_file, device)
        ...
        for out in self.sess.get_outputs():
            self.sess_output.append(out.name)

        self.inference(np.random.randn(h, w, 3))
    ...
    def build_session(self, onnx_file: str, device: str = 'cpu') -> ort.InferenceSession:
    ...
    def inference(self, img: np.ndarray) -> np.ndarray:
        ...
        sess_input = {self.sess_input: input}

        # run model
        outputs = self.sess.run(self.sess_output, sess_input)
    ...
    def postprocess(self, 
        ...
        # rescale keypoints
        keypoints = keypoints / self.model_input_size * scale + center - scale / 2
    ...
    def bbox_xyxy2cs(self, bbox: np.ndarray,
    ...
    def extract_bounding_boxes(self, image_array, bboxes):
        """
        Extracts bounding boxes from an image array and returns a list of cropped images.
        
        Args:
            image_array (numpy.ndarray): The image array.
            bboxes (list of tuple): List of bounding boxes in the format (x_min, y_min, x_max, y_max).
        
        Returns:
            list of numpy.ndarray: List of cropped images corresponding to the bounding boxes.
        """
        cropped_images = []
    ...
    def process(self, image, bboxes, vis_th=0.3):
        """
        Processes an image to extract and analyze keypoints from given bounding boxes.

        Args:
            image (numpy.ndarray): The input image array.
            bboxes (list of tuple): List of bounding boxes in the format (x_min, y_min, x_max, y_max).
            vis_th (float): Visibility threshold for visualizing keypoints.

        Returns:
            tuple: A tuple containing:
                - image (numpy.ndarray): The original image with keypoints visualized.
                - all_keypoints (list): A list of keypoints for each bounding box.
                - all_scores (list): A list of scores for each set of keypoints.
        """
        cropped_images = self.extract_bounding_boxes(image, bboxes)
        all_keypoints = []
        all_scores = []
        for i, cropped_image in enumerate(cropped_images):
            resized_img, center, scale = self.preprocess(cropped_image)
            outputs = self.inference(resized_img)
            keypoints, scores = self.postprocess(outputs, center, scale)
            keypoints = keypoints + np.array([bboxes[i][:2]])
            all_keypoints.append(keypoints[0].tolist())
            all_scores.append(scores[0].tolist())
            self.visualize(image, keypoints, scores, vis_th)

        return image, all_keypoints, all_scores
...
if __name__ == "__main__":
    ...
    h, w, _ = image.shape
    rtmpose = RTMPose(onnx_file)
    out_img, all_kpts, all_scores = rtmpose.process(image, [[0, 0, w, h]])
    cv2.imwrite("output.jpg", out_img)
    for kpts, scores in zip(all_kpts, all_scores):
        for i, kpt, score in zip(range(len(kpts)), kpts, scores):
            print(f"{i}\t[{kpt[0]:.2f}, {kpt[1]:.2f}]\t{score:.2f}")
